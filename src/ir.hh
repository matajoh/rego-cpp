...
namespace rego::ir
{
  ...
  inline const auto BuiltInFunction =
    TokenDef("rego-ir-builtinfunction", flag::lookup | flag::symtab);
  inline const auto Decl = TokenDef("rego-ir-decl");
  inline const auto Plan =
    TokenDef("rego-ir-plan", flag::lookup | flag::symtab);
  inline const auto Block = TokenDef("rego-ir-block");
  inline const auto Function =
    TokenDef("rego-ir-function", flag::lookup | flag::symtab);
  ...
  inline const auto ObjectInsertOnceStmt =
    TokenDef("rego-ir-objectinsertoncestmt");
  ...
  inline const auto Statement = ArrayAppendStmt | AssignIntStmt |
  ...
  {
  public:
    VirtualMachine(Node ir);
    Node run(const Location& plan, Node input);

  private:
    Node read_local(const Location& key);
    void write_local(
      const Location& key, const Node& value, bool once = false);
    bool is_defined(const Location& key);
    void reset_local(const Location& key);
    Node unpack_operand(const Node& operand);
    void run_block(Node block);
    void run_stmt(Node stmt);
    Node call_function(const Location& func_name, const Node& args);

    Node m_input;
    Node m_data;
    std::map<Location, Node> m_locals;
    std::map<Location, Node> m_plans;
    std::map<Location, Node> m_functions;
    std::vector<std::string> m_strings;
    std::vector<Node> m_result_set;
  };
}