...
namespace rego
{
  ...
  /// @brief Attempts to extract the value of a node as an 64-bit floating point number.
  /// @param node The node to extract from.
  ...
  /// @details The resulting string will have any enclosing quotes removed.
  /// @param node The node to extract from.
  /// @return The string value of the node.
  std::string get_string(const Node& node);

  /// @brief Attempts to extract the value of a node as a string.
  /// @details The resulting string will have any enclosing quotes removed.
  /// @param node The node to extract from.
  /// @return The string value of the node
  ...
  /// @brief Attempts to extract the value of a node as a boolean.
  /// @param node The node to extract from.
  /// @return The boolean value of the node.
  ...
  /// @brief Returns the raw location of the node from the source
  /// @note If this is a synthetic node (i.e., one generated by the compiler) then this may be an empty string.
  /// @param node
  /// @return The raw string from the original source for this Node
  ...
  /// @details
  /// The Output class encapsulates the output of a Rego query. A Rego query
  /// result can be either undefined (no results), one or more errors, or
  /// one or more results. The fact that a query can result in one or more results
  /// may seem surprising. The query `a = [1, 2, 3, 4]; x = a[_]; x % 2 == 0; x / 2` results
  /// in two distinct solutions:
  /// ```json
  /// {"expressions":[true, true, true, 1], "bindings":{"a":[1,2,3,4], "x":2}}
  /// {"expressions":[true, true, true, 2], "bindings":{"a":[1,2,3,4], "x":4}}
  /// ```
  /// Each result consists of a list of expressions (one for each statement in the query) and a
  /// lookup table of bindings, with assigned values for each named local. This class provides
  /// methods for accessing these value. Using the output above:
  /// 
  class Output
  {
    Node m_node;
  ...
  public:
    ...
    /// @brief Executes a query against the interpreter.
    /// @details
    /// This method calls Interpreter::query_node and then wraps it in an Output object.
    /// @return The result of the query.
